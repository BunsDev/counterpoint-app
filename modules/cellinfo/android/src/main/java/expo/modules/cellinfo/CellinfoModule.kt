...
class CellinfoModule : Module() {
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("getAllCellInfo") {
      val cellInfoList: List<CellInfo>?  = telephonyManager()?.getAllCellInfo()
           if (cellInfoList != null) {
            val cellInfoArray = CellInfoConverter.createAllCellInfoArray(cellInfoList)
            return@AsyncFunction cellInfoArray
        } else {
            throw Exception("Unable to get cell info")
        }
    }
